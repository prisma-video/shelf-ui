...
import AID "./motoko/AccountIdentifier";
import ExtCore "./motoko/Core";
...
shared (install) actor class movieNFT(mintingAuthority: Principal) = this {
  ...
  private var _allowances : HashMap.HashMap<TokenIndex, Principal> = HashMap.fromIter(_allowancesState.vals(), 0, ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
	...
	private stable var _tokenMetadataState : [(TokenIndex, Blob)] = [];
  private var _tokenMetadata : HashMap.HashMap<TokenIndex, Blob> = HashMap.fromIter(_tokenMetadataState.vals(), 0, ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  ...
  public shared(msg) func mintNFT(to : Principal, metadata : Text) : async TokenIndex {
		assert(msg.caller == _mintingAuthority);
    let receiver = AID.fromPrincipal(to, null);
  ...
  public query func balance(request : BalanceRequest) : async BalanceResponse {
	...
	public query func ownerOf(token : TokenIdentifier) : async Result.Result<AccountIdentifier, CommonError> {
	...
	public query func totalSupply(token : TokenIdentifier) : async Result.Result<Balance, CommonError> {
    #ok(_supply);
  };
  ...
  public query func getTokens() : async [(TokenIndex, Blob)] {
  ...
  public query (msg) func getNFTsOfOwner() : async [(TokenIndex, Principal)] {
    var _tokens: [(TokenIndex, Principal)] = [];
    for ((token, owner) in _allowances.entries()) {
      if (owner == msg.caller) {
        _tokens := Array.append<(TokenIndex, Principal)>(_tokens, [(token, owner)]);
      };
    };
    _tokens
  ...
  // public query (msg) func tokenOfOwnerByIndex() : async [(TokenIndex, Principal)] {
  //   Iter.toArray(HashMap.mapFilter(_tokenMetadata, ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash, (token, owner) : boolean => {owner == msg.caller}).entries());
  // };

  public query func metadata(token : TokenIdentifier) : async Result.Result<Blob, CommonError> {
    if (ExtCore.TokenIdentifier.isPrincipal(token, Principal.fromActor(this)) == false) {
			return #err(#InvalidToken(token));
		};
		let tokenind = ExtCore.TokenIdentifier.getIndex(token);
